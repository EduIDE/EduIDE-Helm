...
| versions.landingPage | string | `"1.2.2"` | EduIDE-Landing-Page. Released independently as well. 1.2.2 is the first release whose privacy page reads the `landingPage.privacy` block below. Pinning it here is what makes that block take effect - an older image ignores it and keeps its hardcoded text. |
