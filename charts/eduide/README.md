...
| versions.landingPage | string | `"1.2.2"` | EduIDE-Landing-Page. Released independently as well. 1.2.2 is the first release whose privacy page reads the `landingPage.privacy` block below. 2.2.0 shipped that block while still pinning 1.2.1, which ignores it, so every installation kept the hardcoded text the block exists to replace. |
